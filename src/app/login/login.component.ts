...
export class LoginComponent implements OnInit {
  ...
  constructor(public data:DataService,public router:Router,@Inject(LOCAL_STORAGE) private storage:WebStorageService,public location: Location) { }
        ...
        for(var i=0;i<this.listData.length;i++){
          if(username==this.listData[i].name && password==this.listData[i].password){
           ...
           this.storage.set('vicky_id',this.listData[i].uid);
            console.log(this.storage.get('vicky_id'));
